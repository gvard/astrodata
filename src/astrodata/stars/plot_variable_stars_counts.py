...
parser = ArgumentParser(
    description="Graph plotter script with distribution of Solar system objects"
    )
...
vsx_vars = int(
    soup.findAll("table")[1].findAll("td")[5].find("b").text.replace(",", "")
)
...
FNAM = f"variable-stars-counts{LOC}"
TMP_PTH = PLOT_PTH + f"{FNAM}_.{FILE_EXT}"
plt.savefig(TMP_PTH, dpi=120)
if FILE_EXT == "svg":
    optimize_svg(TMP_PTH, PLOT_PTH + f"{FNAM}.svg")
    os.remove(TMP_PTH)
...
FNAME = f"gcvs-variable-stars-counts{LOC}"
