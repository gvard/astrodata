...
for i, year in enumerate(years_to_plot):
    ...
    if args.verbose:
        print(
            f"За {year} год {grbnums[i]} локализованных GRB, {xa_dct[year]} "
            f"послесвечений в рентгене, {opts_dct[year]} оптических послесвечений, "
            f"{ra_dct[year]} в радио, {rs_dct[year]} красных смещений. Всего "
            f"к концу года {all_grb_count} GRB, {opt_count} послесвечений")
...
JSON_FILENAME = "../../../data/grbs-stats/index.json"

with open(JSON_FILENAME, 'w', encoding='utf8') as json_file:
    json.dump(result_dct, json_file, indent=None, separators=(',', ': '),
              ensure_ascii=False)
