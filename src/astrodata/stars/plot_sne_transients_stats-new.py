...
today = datetime.now()
DAY, MONTH, MONTH2, YEAR = today.day, today.strftime("%B"), today.strftime("%m"), today.year
DAY, MONTH, MONTH2, YEAR = today.strftime("%d"), today.strftime("%B"), today.strftime("%m"), today.year
...
with open(JSON_FILENAME, 'w', encoding='utf8') as json_file:
    json.dump(sne_stats_dct, json_file, indent=None, separators=(',', ': '),
              ensure_ascii=False)
